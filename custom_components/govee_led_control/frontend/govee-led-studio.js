...
class GoveeLedStudio extends HTMLElement {
  ...
  _styles() {
    return `
      :host { --accent:#7857ff; --accent2:#21d4fd; display:block; }
      ha-card { overflow:hidden; padding:0; background:linear-gradient(145deg, rgba(20,22,31,.98), rgba(9,10,16,.98)); color:#f7f7fb; border:1px solid rgba(255,255,255,.09); }
      ...
      button,select,input { font:inherit; } button { border:0; cursor:pointer; color:#e8e9ef; background:rgba(255,255,255,.07); border-radius:10px; padding:10px 13px; transition:.15s ease; } button:hover { background:rgba(255,255,255,.13); transform:translateY(-1px); } button:disabled { opacity:.38; cursor:not-allowed; transform:none; }
      ...
      .stage { display:grid; grid-template-columns:minmax(280px,1fr) minmax(190px,260px); gap:24px; align-items:start; }
      ...
      .button-grid { display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-bottom:15px; } .primary { width:100%; background:linear-gradient(135deg,var(--accent),#5b8cff); font-weight:700; margin-top:7px; } .danger { width:100%; color:#ff8994; margin-top:8px; }
